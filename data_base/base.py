...
class DB:
    ...
    def create_table(self):
        ...
        genre_id INTEGER
        FOREIGN KEY (genre_id) REFERENCES cateegories(id) 
        )
        ''')
    ...
    def populate_tables(self):
        ...
        ("Дж. Р. Р. Толкин", "Властелин Колец", "1954-1955", "Эпическая сага о путешествии фродо бэггинса, чтобы уничтожить Кольцо Всевластья.", 3),
    ...
    def get_books_by_genre(self, genre):
        self.cursor.execute('''
        SELECT b.name, c.genre FROM name AS r 
        JOIN genre AS c ON r.genre_id = c.id
        WHERE c.genre = :genre
        ''',
        {'genre': genre})
